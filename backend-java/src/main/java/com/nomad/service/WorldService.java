...
import com.nomad.config.BlockchainProperties;
import com.nomad.domain.entity.WorldTile;
import com.nomad.domain.entity.WorldTileId;
import com.nomad.domain.repository.WorldTileRepository;
...
public class WorldService {
    ...
    public String recordDeathOnChain(String victim, String killer) throws Exception {
        String contractAddress = props.getContracts().getGameCore();
        if (contractAddress == null || contractAddress.isBlank()) {
            log.warn("GameCore address not configured, simulating death TX");
            return "0xsimulated_death_" + System.currentTimeMillis();
        }

        Function function = new Function(
                "recordDeath",
                List.of(new Address(victim), new Address(killer)),
        ...
        RawTransaction rawTx = RawTransaction.createTransaction(
                BigInteger.valueOf(props.getChainId()), nonce, gasLimit,
        ...
        if (response.hasError())
            throw new RuntimeException("Death TX error: " + response.getError().getMessage());

        log.info("Death on-chain: victim={} txHash={}", victim, response.getTransactionHash());
